...
def find_transactions(collection: str, query: dict = {}):
    try:
        ...
        raise HTTPException(
            detail=f"Error finding transactions: {str(e)}",
            status_code=400
        )


def find_distinct(collection: str, field: str):
    try:
        result = db_manager.find_distinct(field, collection)
        return result
    except Exception as e:
        raise HTTPException(
            detail=f"Error finding distinct fields: {str(e)}",
            status_code=400
        )

# def create_transaction(transaction: dict, collection: str):
#     try :
#         result = subprocess.check_output(["node", "dbOperations.js",
# "insert", collection, json.dumps(transaction)])
...
#     except Exception as e:
#         raise HTTPException(detail=f"Error processing PDF: {str(e)}",
# status_code=400)

# def create_transactions_bulk(transactions: List[dict], collection: str):
#     try :
#         result = subprocess.check_output(
# ["node", "dbOperations.js", "insert",
#  collection, json.dumps(transactions)])
#         results_arr = result.decode().split('}\n')
#         if(len(results_arr) > 1) :
#             result_value = results_arr[0]
#             result_value = result_value.replace("\n", '').replace("'", '"')
...
# def update_transaction(query: dict, update_data: dict, collection: str):
#     result = subprocess.check_output(["node", "dbOperations.js", "update",
# collection, json.dumps(query), json.dumps(update_data)])
#     return {"numReplaced": int(result)}

# def delete_transaction(query: dict, collection: str):
#     result = subprocess.check_output(["node", "dbOperations.js", "delete",
# collection, json.dumps(query)])
#     return {"numRemoved": int(result)}

# def find_transactions(collection: str, query: dict = {}):
#     result = subprocess.check_output(["node", "dbOperations.js", "find",
# collection, json.dumps(query)])
