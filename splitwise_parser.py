...
def extract_transactions_from_splitwise(transactions: List[str]) :
    for transaction in transactions :
        try:
            ...
            tran_type, amount = get_share(transaction['cost'], transaction['you'])
            transaction = create_transaction_with_time(transaction['description'], amount[1:], date, tran_type, "SPLITWISE", time)
            db.create_transaction(transaction, "splitwise")
        except Exception as e:   
            print("Error processing transaction " + str(e))


def get_share(cost: str, you: str) :
    tran_type = "-"
    amount = "0"
    cost = cost.split("\n")
    you = you.split("\n")
